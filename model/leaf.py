...
class Leaf(Node):
    ...
    def __init__(self, name, data_tuple=None, file_name=None, k=12, verbose=0, optimizer=None, epsilon=3):
    ...
    def update_v(self, v_map, hole=True, soft=True):
        for iid in self.item_map.keys():
            if iid in v_map.keys():
                gradient = np.copy(v_map[iid]) - self.item_map[iid]  # 聚合产生的梯度
                if self.hole_aggregate and iid in self.tmp_gradient_map.keys() and hole:  # 挖洞策略，说明更新的v不包括自己上传的梯度，因此要再做一次自己的v更新
                    gradient += self.tmp_gradient_map[iid]
                self.item_map[iid] += gradient
                # self.v_adams_map[iid].theta = self.item_map[iid]
                # self.v_adams_map[iid].theta = self.item_map[iid]
        if self.verbose > 0:
    ...
    def do_train(self, epoch=10, parent_ste=0, parent_epoch=0, init_lr=0.005,
        ...
        for ste in range(epoch):
            for (uid, iid, r) in self.train_data:
                ...
                vg = (e * u_vec) - lambda_2 * v_vec
                ug = (e * v_vec) - lambda_1 * u_vec
                if adam:
                    v_adams_map[iid].minimize_raw(-vg)
                    u_adams_map[uid].minimize_raw(-ug)
                else:
                    self.user_map[uid] += init_lr * (ug)
                    self.item_map[iid] += init_lr * (vg)
